...
import { EmprestimoCreate } from "@services/emprestimoService";
import { useCreateEmprestimo } from "@hooks/useEmprestimo";
import { Formatters } from "@utils/formatters";

// Esquema de validação Zod para o formulário de Empréstimo
...
interface EmprestimoFormProps {
  ...
  initialValues?: Partial<EmprestimoFormValues>;
}

const defaultValues: EmprestimoFormValues = {
  numeroContrato: "",
  cedenteId: 0,
  produtoId: 0,
  dataOperacao: "",
...
export const EmprestimoForm: React.FC<EmprestimoFormProps> = ({
  ...
  const mutation = useCreateEmprestimo({
    onSuccess: (data) => {
  ...
  // Máscaras e helpers
  const handleCurrencyInput = (field: keyof EmprestimoFormValues) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit((values) => mutation.mutate(values as EmprestimoCreate))}
      autoComplete="off"
      noValidate
    >
        ...
        <div className="form-actions">
          ...
          {mutation.isError && (
            <span className="error" role="alert">
              {mutation.error?.message || "Erro ao salvar empréstimo."}
            </span>
          )}
          {mutation.isSuccess && (
