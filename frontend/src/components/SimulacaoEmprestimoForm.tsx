...
import { SimulacaoEmprestimoCreate } from "@services/simulacaoEmprestimoService";
import { useCreateSimulacaoEmprestimo } from "@hooks/useSimulacaoEmprestimo";
import { Formatters } from "@utils/formatters";

// Esquema de validação Zod para Simulação de Empréstimo
const SimulacaoEmprestimoSchema = z.object({
  numeroSimulacao: z.string().min(1, "Número da simulação obrigatório"),
...
type SimulacaoEmprestimoFormValues = z.infer<typeof SimulacaoEmprestimoSchema>;

interface SimulacaoEmprestimoFormProps {
  onSuccess?: (data: any) => void;
  initialValues?: Partial<SimulacaoEmprestimoFormValues>;
}

const defaultValues: SimulacaoEmprestimoFormValues = {
  numeroSimulacao: "",
  cedenteId: 0,
  produtoId: 0,
  dataOperacao: "",
...
export const SimulacaoEmprestimoForm: React.FC<SimulacaoEmprestimoFormProps> = ({
  onSuccess,
  initialValues,
}) => {
  const {
    ...
    setValue,
    watch,
  } = useForm<SimulacaoEmprestimoFormValues>({
    resolver: zodResolver(SimulacaoEmprestimoSchema),
    defaultValues: { ...defaultValues, ...initialValues },
    mode: "onBlur",
  });

  const mutation = useCreateSimulacaoEmprestimo({
    onSuccess: (data) => {
  ...
  // Máscaras e helpers
  const handleCurrencyInput = (field: keyof SimulacaoEmprestimoFormValues) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <form
      className="simulacao-emprestimo-form"
      onSubmit={handleSubmit((values) => mutation.mutate(values as SimulacaoEmprestimoCreate))}
      autoComplete="off"
      noValidate
    >
      <fieldset disabled={isSubmitting || mutation.isPending}>
        <div className="form-row">
          <label>
            ...
            <input
              type="text"
              {...register("numeroSimulacao")}
              autoFocus
              maxLength={30}
              aria-invalid={!!errors.numeroSimulacao}
            />
            {errors.numeroSimulacao && <span className="error">{errors.numeroSimulacao.message}</span>}
          </label>
          <label>
            Cedente (ID) *
            <input
        ...
        <div className="form-actions">
          ...
          {mutation.isError && (
            <span className="error" role="alert">
              {mutation.error?.message || "Erro ao salvar simulação."}
            </span>
          )}
          {mutation.isSuccess && (
...
export default SimulacaoEmprestimoForm;