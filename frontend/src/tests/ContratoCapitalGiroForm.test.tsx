...
// Mock the mutation hook to avoid real HTTP requests
jest.mock("@hooks/useContratoCapitalGiro", () => ({
...
describe("ContratoCapitalGiroForm", () => {
  it("renders all required fields", () => {
    renderWithProviders(<ContratoCapitalGiroForm />);
  ...
  it("shows validation errors when submitting empty form", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Número do contrato obrigatório/i)).toBeInTheDocument();
  ...
  it("calls onSuccess after successful submit", async () => {
    ...
    jest.spyOn(require("@hooks/useContratoCapitalGiro"), "useCreateContratoCapitalGiro").mockReturnValue({
      mutate: mutateMock,
      isPending: false,
      isError: false,
      isSuccess: false,
    ...
    renderWithProviders(<ContratoCapitalGiroForm onSuccess={onSuccess} />);
    fireEvent.change(screen.getByLabelText(/Número do Contrato/i), { target: { value: "12345" } });
